...
public:
  pattern(const std::vector<int> &p, int iso = 8)
     ...
     * if we apply the pattern to the clockwise rotated board (right), we will
     * get 0x2131, which is the same as applying pattern { 12, 8, 4, 0 } to the
     * original board { 0, 1, 2, 3 } and { 12, 8, 4, 0 } are isomorphic patterns
     * +------------------------+       +------------------------+
     * |     2     8   128     4|       |     4     2     8     2|
     ...
     * +------------------------+       +------------------------+
     *
     * therefore if we make a board whose value is 0xfedcba9876543210ull (the
     * same as index) we would be able to use the above method to calculate its
     * 8 isomorphisms
     */
    for (int i = 0; i < 8; i++) {
...
public:
  ...
  virtual std::string name() const {
    return std::to_string(isomorphic[0].size()) + "-tuple pattern " +
           nameof(isomorphic[0]);
  }
...
protected:
  size_t indexof(const std::vector<int> &patt, const board &b) const {
    ...
    for (size_t n = 0; n < patt.size(); n++) {  
      index |= b.at(patt[n]) << (4 * n);  // save the index of pattern, left shift after 4
    }
    return index;
  }
...
public:
  bool operator==(const state &s) const {
    return (opcode == s.opcode) && (before == s.before) && (after == s.after) &&
           (esti == s.esti) && (score == s.score);
  }
  bool operator<(const state &s) const {
    if (before != s.before)
...
public:
  ...
  state select_best_move(const board &b) const {
    ...
    for (state *move = after; move != after + 4; move++) {
      if (move->assign(b)) {
        // TODO
        move->set_value(move->reward() + expect_value(move->after_state()));  //calculate corresponding reward and popup
  ...
  /**
   * update the statistic, and display the status once in 1000 episodes by
   * default
   *
   * the format would be
   ...
   */
  void make_statistic(size_t n, const board &b, int score, std::string name, int unit = 1000) {
    ...
    if (n % unit == 0) { // show the training process
      ...
      for (int t = 1, c = 0; c < unit; c += stat[t++]) {
        ...
        info << "\t" << ((1 << t) & -2u) << "\t" << (accu * coef) << "%";// use -2u (0xff...fe) to remove the unnecessary 1 for (1 << 0)
        info << "\t(" << (stat[t] * coef) << "%)" << std::endl;
        if (t == 11)
          pert = accu * coef;
      }
      std::ofstream out(name, std::ios::app);
      out << mean << "," << pert << "," << std::endl;
      out.close();
      scores.clear();
      maxtile.clear();
    }
  ...
  void load(const std::string &path) {
    ...
    if (in.is_open()) {
      ...
      if (size != feats.size()) {
        error << "unexpected feature count: " << size << " (" << feats.size()
              << " is expected)" << std::endl;
        std::exit(1);
      }
      for (feature *feat : feats) {
...
int main(int argc, const char *argv[]) {
  info << "LAB3-2048" << std::endl;
  learning tdl;

  // set the learning parameters
  float alpha = 0.003125; // 0.1/32
  size_t total = 600000;
  size_t feature2 = 10000000;
  unsigned seed;
  ...
  std::string w_file = "weight_6-5";
  std::string s_file = "./scores/scores_6-5.txt";
  std::string ms_file = "./scores/mean_scores_6-5.txt";
  tdl.load(w_file);
  ...
  for (size_t n = 1; n <= total; n++) {
    ...
    while (true) {
      ...
      state best = tdl.select_best_move(b);  //select best move
      path.push_back(best);

      if (best.is_valid()) {
        debug << "best " << best;
        score += best.reward();  // update score
        b = best.after_state();
        b.popup();
      } else {
        break;
    ...
    // std::ofstream out(s_file, std::ios::app);
    // out << score << std::endl;
    // out.close();
    // update by TD(0)
    tdl.update_episode(path, alpha);  //
    tdl.make_statistic(n, b, score, ms_file);
    path.clear();
  }
