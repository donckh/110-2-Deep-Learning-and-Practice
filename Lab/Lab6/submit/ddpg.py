...
class DDPG:
    ...
    def _update_behavior_network(self, gamma):
        ...
        actor_loss = -critic_net(state, action).mean()  # min q_value: select max = -min
    ...
    def _update_target_network(target_net, net, tau):
        ...
        for target, behavior in zip(target_net.parameters(), net.parameters()):
            ## TODO ##
            target.data.copy_(tau * behavior + (1 - tau) * target)  # update network parameter from Q(behavior) and Q(target)
