...
def reparameter(mean, logvar, size, device):
    ...
    eps = torch.randn_like(std)  # normal distribution with shape of std
    return mean + eps * std


def plot_output(ref_seq, norm_seq, rec_seq, args, epoch):
    # print('=========================start_plot_rec===================================')
    path = '%s/gif/' % args.log_dir
    path_pic = '%s/pic/' % args.log_dir
    os.makedirs(path, exist_ok=True)
    os.makedirs(path_pic, exist_ok=True)
    frame_num = args.n_past + args.n_future
    str_ref_seq, str_norm_seq, str_rec_seq = 'ref_seq', 'norm_seq', 'rec_seq'

    for batch in range(args.batch_size):
        filenames1, filenames2, filenames3 = [], [], []
        for frame in range(frame_num):
            filename1 = f'{str_ref_seq}_{epoch}_{batch}_{frame}.png'
            # print('plot_size: ', ref_seq.size(), ref_seq[batch, frame].size())
            show_from_tensor(ref_seq[frame, batch], path_pic, filename1)  # save fig
            # create file name and append it to a list
            filenames1.append(path_pic + filename1)

            filename2 = f'{str_norm_seq}_{epoch}_{batch}_{frame}.png'
            # print('plot_size: ', pred_seq.size(), pred_seq[batch, frame].size())
            show_from_tensor(norm_seq[frame, batch], path_pic, filename2)  # save fig
            # create file name and append it to a list
            filenames2.append(path_pic + filename2)

            filename3 = f'{str_rec_seq}_{epoch}_{batch}_{frame}.png'
            # print('plot_size: ', pred_seq_norm.size(), pred_seq_norm[batch, frame].size())
            show_from_tensor(rec_seq[frame, batch], path_pic, filename3)  # save fig
            # create file name and append it to a list
            filenames3.append(path_pic + filename3)
            # plt.axis("off")
            # plt.yticks([])
            # plt.imshow(image)
            # # save frame
        ...
        # build gif
        with imageio.get_writer('%s%s_%d_%d.gif' % (path, str_ref_seq, epoch, batch), mode='I') as writer:
            for filename1 in filenames1:
                image1 = imageio.imread(filename1)
                writer.append_data(image1)

        with imageio.get_writer('%s%s_%d_%d.gif' % (path, str_norm_seq, epoch, batch), mode='I') as writer:
            for filename2 in filenames2:
                image2 = imageio.imread(filename2)
                writer.append_data(image2)

        with imageio.get_writer('%s%s_%d_%d.gif' % (path, str_rec_seq, epoch, batch), mode='I') as writer:
            for filename3 in filenames3:
                image3 = imageio.imread(filename3)
...
def pred_norm(validate_seq, validate_cond, modules, args, device):
    ...
    for i in range(args.n_past + args.n_future):
        encoder_output_total.append(modules['encoder'](validate_seq[i]))  # h5.view(-1, self.dim), [h1, h2, h3, h4]
    for i in range(1, args.n_past + args.n_future):  # frames_size
        # if i < args.n_past or use_teacher_forcing:  # use_teacher_forcing
        ...
        lstm_input = torch.cat((encoder_output_past, validate_cond[i], z), 1)
...
def pred_rec(validate_seq, validate_cond, modules, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):  # frames_size
        ...
        if args.last_frame_skip or i < args.n_past:
            ...
            encoder_output_past = modules['encoder'](decoder_output_total[i-1])[0]  # h5, [h1, h2, h3, h4] take h5 only
            # print('encoder_len: ', len(modules['encoder'](decoder_output_total[i])))
            # mean, logvar = args.mean, torch.tensor(args.var).to(device)
            # z = reparameter(mean, logvar, (args.batch_size, args.z_dim), device)
