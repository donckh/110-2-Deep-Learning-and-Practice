...
from utils_backup_v3 import init_weights, kl_criterion, finn_eval_seq, plot_output, pred_rec, pred_norm, reparameter
...
def parse_args():
    ...
    parser.add_argument('--tfr_start_decay_epoch', type=int, default=40, help='The epoch that teacher forcing ratio become decreasing')  # 0
...
def train(x, cond, modules, optimizer, args, device):
    ...
    for i in range(1, args.n_past + args.n_future):  # frames_size
        ...
        # print('decoder_output_total: ', decoder_output_total.size(), 'decoder_output_total[i - 1]: ', decoder_output_total[i - 1].size())
...
def main():
    ...
    if args.model_dir != '':
        ...
        name = 'v3_3_rnn_size=%d-predictor-posterior-rnn_layers=%d-%d-n_past=%d-n_future=%d-lr=%.4f-g_dim=%d-z_dim=%d-last_frame_skip=%s-beta=%.7f' \
